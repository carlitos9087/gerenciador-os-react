...
// "Tester" é o nome no frontend para o perfil Executante (valor 4) do backend.
// O backend espera receber a string "Executante" — veja perfilParaApi() abaixo.
const PERFIS = ['Emitente', 'Gerente', 'Administrador', 'Tester'];
const SETORES = ['Garantia de Validação', 'Mecatrônica', 'Operação', 'Nenhum'];

// Converte o label do front ("Tester") para o valor que a API aceita ("Executante")
function perfilParaApi(label) {
  if (label === 'Tester') return 'Executante';
  return label;
}

// Converte o valor que vem da API ("Executante") para o label do front ("Tester")
function perfilParaExibir(valor) {
  if (String(valor || '').toLowerCase() === 'executante') return 'Tester';
  return valor || '';
}
...
export default function UsuariosPage({ usuario, showAlert }) {
  ...
  useEffect(() => {
    ...
    const f = usuarios.filter(u => {
      const okB = !b || (u.Nome || u.nome || '').toLowerCase().includes(b) || (u.Email || u.email || '').toLowerCase().includes(b);
      // Compara usando o label de exibição para que o filtro "Tester" funcione
      const perfilExibido = perfilParaExibir(u.Perfil || u.perfil || '');
      const okP = !perfilFilter || perfilExibido.toLowerCase() === perfilFilter.toLowerCase();
      const setorApi = setorParaApi(setorFilter);
      const okS = !setorFilter || String(u.Setor || u.setor || '').toLowerCase() === setorApi.toLowerCase();
      return okB && okP && okS;
    });
  ...
  function abrirModal(id) {
    ...
    setModal({
      ...
      email: u ? (u.Email || u.email || '') : '',
      senha: '',
      // Converte "Executante" vindo da API para "Tester" no formulário
      perfil: u ? perfilParaExibir(u.Perfil || u.perfil || 'Emitente') : 'Emitente',
      setor: u ? exibirSetor(u.Setor || u.setor) : 'Garantia de Validação',
    });
  }
  ...
  async function salvarUsuario() {
    ...
    if (!id && !senha) { alert('Informe a senha.'); return; }
    const editando = !!id;
    // Converte "Tester" → "Executante" antes de enviar para a API
    const perfilApi = perfilParaApi(perf);
    try {
      let r;
      if (editando) {
        const body = senha
          ? { nome, email, senha, perfil: perfilApi, setor: setorParaApi(setor) }
          : { nome, email, perfil: perfilApi, setor: setorParaApi(setor) };
        r = await api('PUT', `/usuarios/${id}`, body);
      } else {
        r = await api('POST', '/usuarios', { adminId: userId(usuario), nome, email, senha, perfil: perfilApi, setor: setorParaApi(setor) });
      }
      if (r.ok) {
        setModal(null);
        showAlert(editando ? 'Usuário atualizado!' : 'Usuário criado!', 'success');
              ...
              <table>
                ...
                <tbody>
                  {filtered.map(u => {
                    const id = u.Id || u.id;
                    const nome = u.Nome || u.nome || '';
                    // Exibe "Tester" no lugar de "Executante"
                    const perf = perfilParaExibir(u.Perfil || u.perfil || '');
                    const setor = exibirSetor(u.Setor || u.setor);
                    return (
                      <tr key={id}>
                        <td>{id}</td>
