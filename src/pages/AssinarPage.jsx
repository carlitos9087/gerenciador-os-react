...
import { useState, useEffect } from 'react';
import { api, perfil, userId, isTester, userSetor, exibirSetor, userName, jaAssinou, totalAssinaturas, URL_BASE, getToken } from '../api';
import { SignaturesBadge, Spinner, Empty } from '../components/UI';
import OSDetailPanel from '../components/OSDetailPanel';
...
export default function AssinarPage({ usuario, showAlert }) {
  ...
  // Tester vê todas as OSs; Gerente vê as do seu setor
  const urlOsc = tester ? '/osc' : `/osc/gerente/${userId(usuario)}`;
  ...
  async function assinarOS(os) {
    // Tester não pode assinar (só visualiza)
    if (tester) {
      showAlert('Tester não tem permissão para assinar.');
      return;
    }
    if (!confirm(`Confirma assinatura da OS-${String(os.id).padStart(3, '0')}?\n\n${os.descricao}`)) return;
    try {
  ...
  return (
    <div className={selectedOs ? 'panel-open' : ''} style={{ position: 'relative' }}>
      ...
      <div className="page-sub">
        {tester
          ? <span>👁️ Visualização — <strong>Tester</strong> · {userName(usuario)} <span style={{ fontSize: 12, color: '#aaa' }}>· somente leitura</span></span>
          : <>OSs pendentes — {setor} · {userName(usuario)}{' '}<span style={{ fontSize: 12, color: '#aaa' }}>· clique em uma linha para ver detalhes</span></>
        }
      </div>
